...
import { login, register, getMe, googleLoginApi } from "../api/api";
...
export const googleLogin = (credentials: any) => {
  return async (dispatch: Dispatch) => {
    const response = await googleLoginApi(credentials);
...
export const registerUser = (credentials: RegisterArgs) => {
  return async (dispatch: Dispatch) => {
    ...
    if ("error" in response) {
      ...
      dispatch(
        showAlert(
          `Hi 👋 ${response.userDetails.username}. Welcome to MeT. I'm bot-tu, an assistant of MeT. You have me a as a friend till you invite and add your other friends 😊.`
        )
      );
    }
  };
